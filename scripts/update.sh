...
if [[ "$modules_before" != "$modules_after" ]]; then
  if ! sudo nixos-rebuild switch --impure --flake /etc/nixos/; then
    "$dunstify_bin" --urgency=critical "nixos-rebuild" "update.sh: nix-modules changed, rebuild failed"
  fi
fi

if [[ "$hm_before" != "$hm_after" ]]; then
  if ! home-manager switch --impure --flake ~/.home-manager/; then
    "$dunstify_bin" --urgency=critical "home-manager" "update.sh: home-manager changed, switch failed"
  fi
fi
