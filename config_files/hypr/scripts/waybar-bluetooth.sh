...
if ! command -v bluetoothctl >/dev/null 2>&1; then
  printf '{"text":"","class":"off","tooltip":"bluetoothctl not found"}\n'
  exit 0
fi

powered="$(bluetoothctl show 2>/dev/null | awk -F': ' '/Powered:/{print $2; exit}')"
if [[ "${powered:-no}" != "yes" ]]; then
  printf '{"text":"","class":"off","tooltip":"Bluetooth is powered off"}\n'
  exit 0
fi
...
printf '{"text":"","class":"idle","tooltip":"Bluetooth is on"}\n'
