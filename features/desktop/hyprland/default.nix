...
let
  ...
  repoDir = "${config.home.homeDirectory}/.home-manager";
  configDir = "${repoDir}/features/desktop/hyprland/config";
  hostDir = "${repoDir}/hosts/${config.host.name}";
...
in
{
  config = lib.mkIf config.desktopEnv.enable {
    ...
    # the system side; see /etc/nix-modules/desktop_env/hyprland.nix.
    xdg.portal = {
      enable = true;
