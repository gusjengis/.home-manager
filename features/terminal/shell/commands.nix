...
let
  ...
  resolveHost = ''
    ...
    if [ -z "$host" ]; then
      echo "$command_name: this machine's id is not in hosts/default.nix." >&2
      ...
      echo "        Run '$command_name <host>', then record the machine-id in hosts/default.nix." >&2
      exit 1
    fi
  '';
